...
def generate_ffmpeg_cmd(input_path: str, output_path: str, speed: float, keep_pitch: bool = True) -> str:
    ...
    # Audio filter: atempo handles speeds between 0.5 and 2.0; chain if necessary
    audio_filters = []
    curr = speed
    while curr > 2.0:
...
def main():
    ...
    print(f"Target Speed      : {args.speed:.2f}x")
    print(f"New Duration      : {new_sec/60:.1f} mins ({new_sec:.0f}s)")
    print(f"Time Saved        : {saved_sec/60:.1f} mins ({saved_sec:.0f}s)")
    print("-" * 60)
    print("Recommended FFmpeg Command:")
